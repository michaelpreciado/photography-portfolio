...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- Cinematic scroll reveals (.reveal -> .is-visible) --- //
    const revealObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // Stagger siblings that reveal together for an editorial feel.
    ...
    async function loadPortfolioImagesByCategory(category, gridElement) {
        ...
        for (const mediaEntry of shuffledMedia) {
            ...
            // Stagger index for the cinematic entrance animation (capped so
            // later tiles don't wait excessively).
            gridItem.style.setProperty('--i', String(tileIndex % 12));
            tileIndex++;

            const isVideo = mediaEntry.type === 'video' || /\.(mp4|webm|mov|m4v)$/i.test(mediaPath);

            // No CDN source wired yet: render the premium poster/placeholder
            // card instead of an empty <video> so the Visuals tab still reads
            // as intentional. Lights up automatically once a src is provided.
            if (isVideo && !mediaPath) {
                const fallback = document.createElement('div');
                fallback.className = 'media-fallback is-video';
                fallback.setAttribute('role', 'img');
                fallback.setAttribute('aria-label', mediaEntry.alt || 'Visual coming soon');
